const { mergeWhereClause, upcase } = require("@ocopjs/utils");

const {
  throwAccessDenied,
  ValidationFailureError,
} = require("../ListTypes/graphqlErrors");
const { graphqlLogger } = require("../Ocop/logger");
...
class ListAuthProvider {
  constructor({ authStrategy, list, hooks }) {
    ...
    this.gqlNames = {
      ...
      authenticateMutationName: `authenticate${itemQueryName}With${upcase(
  ...
  getTypes({ schemaName }) {
    ...
    const { unauthenticateOutputName, authenticateOutputName, outputTypeName } =
      this.gqlNames;
    return [
      `
  ...
  getMutations({ schemaName }) {
    ...
    const updateFields = this.list.getFieldsWithAccess({
      schemaName,
      access: "update",
    });
  ...
  getQueryResolvers({ schemaName }) {
    ...
    return {
      [authenticatedQueryName]: (_, __, context, info) =>
        this._authenticatedQuery(context, info),
    };
  }
  getMutationResolvers({ schemaName }) {
    ...
    const ret = {
      [authenticateMutationName]: (_, args, context) =>
        this._authenticateMutation(args, context),
      [unauthenticateMutationName]: (_, __, context) =>
        this._unauthenticateMutation(context),
    };
    const updateFields = this.list.getFieldsWithAccess({
      schemaName,
      access: "update",
  ...
  async _authenticateMutation(originalInput, context) {
    ...
    await this.hookManager.validateAuthInput({
      operation,
      originalInput,
      resolvedData,
      context,
    });
    await this.hookManager.beforeAuth({
      operation,
      originalInput,
      resolvedData,
      context,
    ...
    const { item, success, message } = await this.authStrategy.validate(
      resolvedData,
      context,
    );
    if (!success) {
  ...
  async _unauthenticateMutation(context) {
    ...
    await this.hookManager.afterUnauth({
      operation,
      context,
      success,
      listKey,
  ...
  async checkAccess(context, type, { gqlName }) {
    ...
    const access = await context.getAuthAccessControlForUser(
      this.list.access,
      this.list.key,
      {
        gqlName,
