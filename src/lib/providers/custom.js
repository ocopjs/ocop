...
const { parseCustomAccess } = require("@ocopjs/access-control");
const { objMerge } = require("@ocopjs/utils");
...
class CustomProvider {
  ...
  extendGraphQLSchema({
    ...
    this._extendedQueries = this._extendedQueries.concat(
      queries.map(_parseAccess),
    );
    this._extendedMutations = this._extendedMutations.concat(
      mutations.map(_parseAccess),
    );
  ...
  _customResolver(type) {
    return ({ schema, subscribe, resolver, access, cacheHint }) => {
      const gqlName = gql(`type t { ${schema} }`).definitions[0].fields[0].name
        .value;
