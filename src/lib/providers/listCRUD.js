...
const { flatten, objMerge, unique } = require("@ocopjs/utils");
...
class ListCRUDProvider {
  ...
  getTypeResolvers({ schemaName }) {
          ...
          .map((list) => ({
            ...
            fields: flatten(
              list
                ...
                .map((field) =>
                  Object.keys(field.gqlOutputFieldResolvers({ schemaName })),
                ),
            ),
          }))
    ...
    return {
      ...referenceResolvers,
      ...objMerge(
        this.lists.map((list) => list.gqlAuxFieldResolvers({ schemaName })),
      ),
      ...objMerge(
        this.lists.map((list) => list.gqlFieldResolvers({ schemaName })),
      ),
      JSON: GraphQLJSON,
  ...
  getQueryResolvers({ schemaName }) {
    ...
    return {
      ...
      ...objMerge(
        firstClassLists.map((list) => list.gqlQueryResolvers({ schemaName })),
      ),

      // And the Ocop meta queries must always be available
      [this.gqlNames.listsMeta]: (
        _,
        { where: { key, auxiliary } = {} },
        context,
      ) =>
  ...
  getMutationResolvers({ schemaName }) {
    ...
    return {
      ...objMerge(
        firstClassLists.map((list) => list.gqlAuxMutationResolvers()),
      ),
      ...objMerge(
        firstClassLists.map((list) =>
          list.gqlMutationResolvers({ schemaName }),
        ),
      ),
    };
  }
