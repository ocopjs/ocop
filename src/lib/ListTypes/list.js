...
const { parseListAccess } = require("@ocopjs/access-control");
...
module.exports = class List {
  constructor(
    ...
    this.adminConfig = {
      defaultColumns: nonIdFieldNames
        ? nonIdFieldNames.slice(0, 2).join(",")
        : "id",
    ...
    if (!["object", "function", "undefined"].includes(typeof cacheHint)) {
      throw new Error(
        `List ${label}'s cacheHint must be an object or function`,
      );
    }
    this.cacheHint = cacheHint;

    // Tell Ocop about all the types we've seen
  ...
  initFields() {
    ...
    if (!sanitisedFieldsConfig.id && this.adapter) {
      if (
        typeof this.adapter.parentAdapter.getDefaultPrimaryKeyConfig !==
        "function"
      ) {
        throw new Error(
    ...
    Object.entries(sanitisedFieldsConfig).forEach(([fieldKey, fieldConfig]) => {
      ...
      if (
        typeof adapters === "undefined" ||
        Object.entries(adapters).length === 0
      ) {
        throw new Error(
  ...
  getFieldsRelatedTo(listKey) {
    return this.fields.filter(
      ({ isRelationship, refListKey }) =>
        isRelationship && refListKey === listKey,
    );
  }
  ...
  async checkListAccess(
    ...
    if (!access) {
      ...
      graphqlLogger.info(
        { operation, gqlName, ...extraInternalData },
        "Access Denied",
      );
      // If the client handles errors correctly, it should be able to
      // receive partial data (for the fields the user has access to),
  ...
  async getAccessControlledItem(
    id,
    access,
    { context, operation, gqlName, info },
  ) {
  ...
  async getAccessControlledItems(ids, access, { context, info } = {}) {
    ...
    const remainingAccess = omit(access, [
      "id",
      "id_not",
      "id_in",
      "id_not_in",
  ...
  async listQuery(args, context, gqlName, info, from) {
    const access = await this.checkListAccess(context, undefined, "read", {
      gqlName,
    });
  ...
  async listQueryMeta(args, context, gqlName, info, from) {
    return {
      ...
      // Evaluation takes place in ../Ocop/index.js
      getCount: async () => {
        const access = await this.checkListAccess(context, undefined, "read", {
          gqlName,
        });

        const { count } = await this._itemsQuery(
          mergeWhereClause(args, access),
          {
            meta: true,
            context,
            info,
  ...
  async itemQuery(
    ...
    const operation = "read";
    graphqlLogger.debug(
      { id, operation, type: opToType[operation], gqlName },
      "Start query",
    );
  ...
  async _itemsQuery(args, extra) {
    ...
    if (extra && extra.info && extra.info.cacheControl) {
      switch (typeof this.cacheHint) {
        ...
        case "function":
          const operationName =
            extra.info.operation.name && extra.info.operation.name.value;
  ...
  async _resolveRelationship(
    ...
    const resolvedRelationships = await mapToFields(fields, async (field) => {
      ...
      const { create, connect, disconnect, currentValue } =
        await field.resolveNestedOperations(
          data[field.path],
          existingItem,
          context,
          getItem,
  ...
  async _resolveDefaults({ context, originalInput }) {
    ...
    return {
      ...omitBy(
        defaultValues,
        (path) => typeof defaultValues[path] === "undefined",
      ),
      ...originalInput,
    };
  }
  ...
  async createManyMutation(data, context, mutationState) {
    ...
    const itemsToUpdate = data.map((d) => ({
    ...
    return Promise.all(
      data.map((d) =>
        this._createSingle(d.data, undefined, context, mutationState),
      ),
    );
  }
  ...
  async _createSingle(originalInput, existingItem, context, mutationState) {
    const operation = "create";
    return await this._nestedMutation(
      mutationState,
      context,
      async (mutationState) => {
        const defaultedItem = await this._resolveDefaults({
          context,
          originalInput,
        });
  ...
  async updateMutation(id, data, context, mutationState) {
    ...
    const access = await this.checkListAccess(
      context,
      data,
      operation,
      extraData,
    ...
    return await this._updateSingle(
      id,
      data,
      existingItem,
      context,
  ...
  async deleteManyMutation(ids, context, mutationState) {
    ...
    return Promise.all(
      existingItems.map((existingItem) =>
        this._deleteSingle(existingItem, context, mutationState),
      ),
    );
  }
  ...
  async _deleteSingle(existingItem, context, mutationState) {
    ...
    return await this._nestedMutation(mutationState, context, async () => {
      ...
      return {
        result: existingItem,
        afterHook: () =>
          this.hookManager.afterDelete({ existingItem, context, operation }),
      };
    });
  }
  ...
  getGqlTypes({ schemaName }) {
    ...
    const readFields = this.getAllFieldsWithAccess({
      schemaName,
      access: "read",
    });
    if (
      ...
      types.push(
        ...flatten(
          this.fields.map((field) => field.getGqlAuxTypes({ schemaName })),
        ),
        typeDefine,
        ...
        input ${this.gqlNames.whereInputName} {
          ...
          ${flatten(
            readFields.map((field) =>
              field.gqlQueryInputFields({ schemaName }),
            ),
          ).join("\n")}
        }`,
    ...
    const updateFields = this.getFieldsWithAccess({
      schemaName,
      access: "update",
    });
        ...
        input ${this.gqlNames.updateInputName} {
          ${flatten(
            updateFields.map((field) =>
              field.gqlUpdateInputFields({ schemaName }),
            ),
          ).join("\n")}
        }
      `);
    ...
    const createFields = this.getFieldsWithAccess({
      schemaName,
      access: "create",
    });
        ...
        input ${this.gqlNames.createInputName} {
          ${flatten(
            createFields.map((field) =>
              field.gqlCreateInputFields({ schemaName }),
            ),
          ).join("\n")}
        }
      `);
  ...
  getGqlQueries({ schemaName }) {
    ...
    const queries = flatten(
      this.fields.map((field) => field.getGqlAuxQueries()),
    );
    ...
    if (schemaAccess.read) {
      queries.push(
        ...
        `
        """ Perform a meta-query on all ${this.gqlNames.outputTypeName} items which match the where clause. """
  ...
  getGqlMutations({ schemaName }) {
    ...
    const mutations = flatten(
      this.fields.map((field) => field.getGqlAuxMutations()),
    );
  ...
  gqlAuxFieldResolvers({ schemaName }) {
    ...
    if (
      ...
      return objMerge(
        this.fields.map((field) => field.gqlAuxFieldResolvers({ schemaName })),
      );
    }
    return {};
  }
      ...
      ...objMerge(
        this.fields
          ...
          .map((field) =>
            ...
            mapKeys(
              field.gqlOutputFieldResolvers({ schemaName }),
              (innerResolver) => this._wrapFieldResolver(field, innerResolver),
            ),
          ),
      ),
    };
  ...
  listMeta(context) {
    return {
      ...
      getAccess: () => ({
        ...
        getAuth: () =>
          context.getAuthAccessControlForUser(this.access, this.key, {
            context,
          }),
      }),
  ...
  gqlMutationResolvers({ schemaName }) {
    ...
    if (schemaAccess.create && createFields.length) {
      mutationResolvers[this.gqlNames.createMutationName] = (
        _,
        { data },
        context,
      ) => this.createMutation(data, context);

      mutationResolvers[this.gqlNames.createManyMutationName] = (
        _,
        { data },
        context,
    ...
    if (schemaAccess.update && updateFields.length) {
      mutationResolvers[this.gqlNames.updateMutationName] = (
        _,
        { id, data },
        context,
      ) => this.updateMutation(id, data, context);

      mutationResolvers[this.gqlNames.updateManyMutationName] = (
        _,
        { data },
        context,
    ...
    if (schemaAccess.delete) {
      mutationResolvers[this.gqlNames.deleteMutationName] = (
        _,
        { id },
        context,
      ) => this.deleteMutation(id, context);

      mutationResolvers[this.gqlNames.deleteManyMutationName] = (
        _,
        { ids },
        context,
