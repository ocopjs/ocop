const { omitBy, arrayToObject } = require("@ocopjs/utils");
...
class HookManager {
  ...
  async resolveInput({
    ...
    // passed in or not
    resolvedData = await mapToFields(this.fields, (field) =>
    ...
    resolvedData = omitBy(
      resolvedData,
      (key) => typeof resolvedData[key] === "undefined",
    );
    ...
    if (this.hooks.resolveInput) {
      ...
      if (typeof resolvedData !== "object") {
        throw new Error(
          `Expected ${this.listKey}.hooks.resolveInput() to return an object, but got a ${typeof resolvedData}: ${resolvedData}`,
        );
      }
    }
    ...
    const fieldValidationErrors = this.fields
      .filter(
        (field) =>
          ...
          ((operation === "create" &&
            (resolvedData[field.path] === undefined ||
              resolvedData[field.path] === null)) ||
            (operation === "update" &&
  ...
  async _validateHook({ args, fields, operation, hookName }) {
    ...
    await mapToFields(fields, (field) =>
