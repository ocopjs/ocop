...
const deleteField = (migration, field, _pluralize) =>
  migration
    ? `db.collection('${_pluralize(
        field.listKey,
      )}').updateMany({}, { $unset: { "${field.path}": 1 } })`
...
const moveData = (migration, left, tableName, near, far, _pluralize) =>
  migration
    ? `db.collection('${_pluralize(
        left.listKey,
      )}').find({}).forEach(function(doc){ (doc.${left.path} || []).forEach(function(itemId) { db.${_pluralize(
        tableName,
      )}.insert({ ${near}: doc._id, ${far}: itemId }) } ) });`
    : `    * Create a collection ${c(_pluralize(tableName))} with fields ${c(
        near,
      )} and ${c(far)}\n    * Move the data from ${c(
        `${_pluralize(left.listKey)}.${left.path}`,
      )} into ${c(_pluralize(tableName))}`;

// Postgres operations
const dropTable = (migration, tableName, schemaName) =>
  migration
    ? `DROP TABLE ${schemaName}."${tableName}"`
    : `    * Drop table ${c(tableName)}`;
...
const strategySummary = (
  ...
  const mongo = ocop.adapter.name === "mongoose";
  ...
  two_one_to_many.forEach(({ left, right, tableName }) => {
    ...
    if (mongo) {
      ...
      const { schemaName } = ocop.adapter;
      console.log(
        dropTable(migration, `${dropper.listKey}_${dropper.path}`, schemaName),
      );
    }
  });
  ...
  two_many_to_many.forEach(({ left, right, tableName, columnNames }) => {
    ...
    if (mongo) {
      ...
      const { schemaName } = ocop.adapter;
      console.log(
        dropTable(migration, `${right.listKey}_${right.path}`, schemaName),
      );
      console.log(
        renameTable(
          migration,
...
const upgradeRelationships = async (args, entryFile) => {
  ...
  const rels = ocop._consolidateRelationships();

  const one_many_to_many = rels.filter(
    ({ right, cardinality }) => !right && cardinality === "N:N",
  );

  const two_one_to_one = rels.filter(
    ({ right, cardinality }) => right && cardinality === "1:1",
  );
  const two_one_to_many = rels.filter(
    ({ right, cardinality }) =>
      right && (cardinality === "1:N" || cardinality === "N:1"),
  );
  const two_many_to_many = rels.filter(
    ({ right, cardinality }) => right && cardinality === "N:N",
  );
  ...
  console.log("");
  ttyLink(
    "https://www.ocop.vn/discussions/relationships",
    "More info on relationships",
  );
