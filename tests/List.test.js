...
const config = {
  fields: {
    ...
    writeOnce: {
      type: Text,
      access: { read: true, create: true, update: false, delete: true },
    },
  },
};
    ...
    if (meta) {
      return {
        count: (id !== undefined
          ? [this.items[id]]
          : ids
              .filter((i) => !id_not_in || !id_not_in.includes(i))
              .map((i) => this.items[i])
        ).length,
      };
    } else {
...
describe("new List()", () => {
  ...
  test("new List() - fields", () => {
    ...
    expect(list.fieldsByPath["other"]).toBeInstanceOf(
...
test("labelResolver", () => {
  ...
  const list = setup();
  expect(
    list.labelResolver({ name: "a", email: "a@example.com", id: "1" }),
  ).toEqual("a");
  ...
  expect(
    list2.labelResolver({ name: "a", email: "a@example.com", id: "2" }),
  ...
  expect(
    list21.labelResolver({ name: "Test integer", index: 0, id: "21" }),
  ).toEqual("0");
  ...
  expect(
    list3.labelResolver({ name: "a", email: "a@example.com", id: "3" }),
  ).toEqual("a - a@example.com");
...
describe(`getGqlQueries()`, () => {
  ...
  test("access: true", () => {
    expect(
      setup({ access: true }).getGqlQueries({ schemaName }).map(normalise),
    ).toEqual(
      [
  ...
  test("access: false", () => {
    expect(
      setup({ access: false }).getGqlQueries({ schemaName }).map(normalise),
    ).toEqual([]);
  });
});
...
test("getFieldsRelatedTo", () => {
  const list = setup();
  expect(list.getFieldsRelatedTo("Other")).toEqual([
    list.fieldsByPath["other"],
  ]);
...
test("_wrapFieldResolverWith", async () => {
  ...
  const list = setup();
  const newResolver = list._wrapFieldResolver(
    list.fieldsByPath["name"],
    resolver,
  );
  await expect(newResolver({}, {}, context)).resolves.toEqual("result");
  await expect(newResolver({ makeFalse: true }, {}, context)).rejects.toThrow(
    AccessDeniedError,
  );
});
...
test("checkListAccess", async () => {
  ...
  const originalInput = {};
  await expect(
    list.checkListAccess(context, originalInput, "read", {
      gqlName: "testing",
    }),
  ...
  await expect(
    list.checkListAccess(newContext, originalInput, "update", {
      gqlName: "testing",
    }),
  ).resolves.toEqual(true);
  await expect(
    list.checkListAccess(newContext, originalInput, "read", {
      gqlName: "testing",
    }),
  ).rejects.toThrow(AccessDeniedError);
});
...
test("getAccessControlledItem", async () => {
  const list = setup();
  expect(
    await list.getAccessControlledItem(1, true, {
      context,
      operation: "read",
      gqlName: "testing",
    }),
  ...
  await expect(
    list.getAccessControlledItem(10, true, {
      context,
      operation: "read",
      gqlName: "testing",
    }),
...
test("getAccessControlledItems", async () => {
  ...
  expect(
    await list.getAccessControlledItems([1, 2], { id_in: [1, 2, 3] }),
  ).toEqual([
    { name: "b", email: "b@example.com", index: 1 },
    { name: "c", email: "c@example.com", index: 2 },
  ]);
  expect(
    await list.getAccessControlledItems([1, 2], { id_in: [2, 3] }),
  ).toEqual([{ name: "c", email: "c@example.com", index: 2 }]);
  expect(
    await list.getAccessControlledItems([1, 2], { id_in: [3, 4] }),
  ).toEqual([]);
  ...
  expect(
    await list.getAccessControlledItems([1, 2], { id_not_in: [1, 2, 3] }),
  ).toEqual([]);
  expect(
    await list.getAccessControlledItems([1, 2], { id_not_in: [2, 3] }),
  ).toEqual([{ name: "b", email: "b@example.com", index: 1 }]);
  expect(
    await list.getAccessControlledItems([1, 2], { id_not_in: [3, 4] }),
  ).toEqual([
    { name: "b", email: "b@example.com", index: 1 },
    { name: "c", email: "c@example.com", index: 2 },
  ]);
...
test("listQuery", async () => {
  const list = setup();
  expect(
    await list.listQuery({ where: { id: 1 } }, context, "testing"),
  ).toEqual([{ name: "b", email: "b@example.com", index: 1 }]);
});
...
test("listQueryMeta", async () => {
  ...
  expect(
    await (
      await list.listQueryMeta({ where: { id: 1 } }, context, "testing")
    ).getCount(),
  ).toEqual(1);
  expect(
    await (
      await list.listQueryMeta({ where: { id_in: [1, 2] } }, context, "testing")
    ).getCount(),
  ).toEqual(2);
});
...
test("itemQuery", async () => {
  ...
  await expect(list.itemQuery({ where: { id: 4 } }, context)).rejects.toThrow(
    AccessDeniedError,
  );
});
...
test("createMutation", async () => {
  const list = setup();
  const result = await list.createMutation(
